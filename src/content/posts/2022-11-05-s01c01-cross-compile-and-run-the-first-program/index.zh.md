...
这是 [《从零开始使用自己的工具一步一步构建现代计算机系统》](../2022-10-31-building-a-modern-computer-system-from-scratch-step-by-step) 系列的第一个系列 ———— 《一步一步深入了解 RISC-V 系统》。这个系列的目标是实现一个 RISC-V 汇编器和链接器，以及一门有特色的汇编语言和链接脚本语言。有了汇编器和链接器，我们就有能力生成程序（二进制可执行文件）了，这是我们制作自己的工具的基础。
...
这章主要讲解什么是交叉编译，以及交叉编译了一个 "Hello World!" 程序，然后使用 QEMU 模拟一个不同架构的 Linux 环境来运行交叉编译得到的可执行文件。通过这一章你会发现其实交叉编译也没任何特别的地方，操作方法跟普通编译几乎一样，只是编译出来的结果（可执行文件）我们无法在当前机器上直接运行而已。
