...
这章主要讲解什么是交叉编译，以及交叉编译了一个 "Hello, World!" 程序，然后使用 QEMU 模拟一个不同架构的 Linux 环境来运行交叉编译得到的可执行文件。通过这一章你会发现其实交叉编译也没什么特别的地方，操作方法跟普通编译几乎一样，只是编译出来的结果（可执行文件）我们无法在当前机器上直接运行而已。
